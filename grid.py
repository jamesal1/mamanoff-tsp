...
def euclidean_distance(points, diag_one=False):
    ...
    if diag_one:
        res1 = (ssum.view(1,-1)+diffs+ssum.view(-1,1)+ones)**.5
    else:
        res1 = (ssum.view(1,-1)+diffs+ssum.view(-1,1))**.5
    return res1
...
def test(size=10,eps=1e-7):
    ...
    optimizer = opt.SGD([emb_points],lr=1e-3,momentum=.9)
    print(emb_points)
    ...
    for i in range(10000):
        ...
        # print(ratio)
        torch.nn.utils.clip_grad_value_([emb_points],1)
        optimizer.step()
    emb_points=emb_points.detach().numpy()
    print(emb_points)
